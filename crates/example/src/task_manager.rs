...
mod tests {
    ...
    #[test]
    fn add_task_ignores_empty_and_adds_trimmed_title() {
        let mut app = sample_app();
        app.draft = "   ".to_string();
        app.add_task();
        assert_eq!(app.tasks.len(), 2);

        app.draft = "  new task  ".to_string();
        app.add_task();
    ...
    fn clear_done_removes_only_completed_tasks() {
        ...
        assert_eq!(app.tasks[0].title, "todo");
    }
    ...
    fn parse_task_id_and_summary_text_work() {
        ...
        let app = sample_app();
        assert_eq!(app.summary_text(), "2 tasks (1 done)");
    }

    #[test]
    fn options_disable_resizing_for_task_manager() {
        let app = sample_app();
        let options = app.options();
        assert_eq!(options.window_title, "runo task manager example");
        assert_eq!(options.window_width, 1100);
        assert_eq!(options.window_height, 760);
