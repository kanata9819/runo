...
mod tests {
    ...
    #[test]
    fn options_match_simple_app_defaults() {
        let app = sample_app();
        let options = app.options();
        assert_eq!(options.window_title, "runo example");
        assert_eq!(options.window_width, 1200);
        assert_eq!(options.window_height, 700);
        assert!(options.window_resizable);
    }

    #[test]
    fn constants_are_stable_for_event_wiring() {
        assert_eq!(TITLE_ID, "title");
        assert_eq!(INPUT_NAME_ID, "input.name");
        assert_eq!(COMBO_ROLE_ID, "combo.role");
        assert_eq!(CHECKBOX_NEWSLETTER_ID, "check.newsletter");
        assert_eq!(RADIO_CHANNEL_GROUP, "channel");
        assert_eq!(SLIDER_VOLUME_ID, "slider.volume");
        assert_eq!(TOGGLE_BUTTON_ID, "btnToggle");
        assert_eq!(MAIN_PANEL_ID, "main.panel");
    }
}
